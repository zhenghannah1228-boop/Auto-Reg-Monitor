...
def build():
    ...
    asean_l3=asean_fitment("MS-BST")
    def l3_of(s,t):
        if s=="ASEAN" and t: return asean_l3
        return {"_status":"TO_EXTRACT" if t else "NOT_TESTED"}
    systems={s:{"version":"","source_files":src.get(s,[]),"L1_subtests":{"盲点监测BST":t},
        "L2_params":({"_note":"盲点检测+(ASEAN另含可视化Visualization)"} if t else {}),
        "L3_thresholds":l3_of(s,t)} for s,t in L1.items()}
    ...
    res=[(L1["ASEAN"],"ASEAN.盲点=测",True,True),(L1["ANCAP"],"ANCAP.盲点=测",True,True),
         ...
         (asean_l3.get("_TFS")==8.0,"ASEAN.L3.BST装备率TFS=8",asean_l3.get("_TFS"),8.0)]
