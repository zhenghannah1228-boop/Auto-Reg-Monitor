...
| **K-NCAP** | 韩国 | **现行方案(年度未在本批源标注)** | 公开"평가방법(评价方法)"说明套件 | 含 Adv BMS 电池安全/车对车 THOR/远侧/ISA/ESF 等新增项→判为 2025+ 方案;韩国国土交通部(MOLIT)/KATRI |
...
## K-NCAP(韩国)接入说明(2026-06 业主提供 KNCAP.zip 14 份官方资料)

- **来源性质**:韩国 KNCAP 公开「평가방법(评价方法)」说明 PDF 套件(国土交通部/KATRI),全部含可提取文字。
- **可拆深度**:**L1(测试项)+ L2(速度/假人/壁障/测量人体部位)已据实录入**;但这批为公开评价方法说明,**仅列测量部位、不含 L3 数值限值** → L3 一律标注「本批未给数值,见 KATRI 详细试验/评分规程(不在本批)」。**严禁臆造**,未编造任何阈值。
- **覆盖 14/16 项**(矩阵新增第 9 列 K-NCAP):
  - 正面全宽(56km/h 固定壁·全席 5%ile HIII)、车对车50%偏置(56km/h·THOR 驾驶+Q6/Q10)、侧碰(60km/h·1400kg MDB·90°·WorldSID)+远侧、柱碰(32km/h·75°斜·WorldSID)、鞭打(后碰·BioRID II·静+动)、eCall(信息提供)、电动车 Adv BMS(热失控感知/警告申报/信息存储)、先进气囊+儿童(Q6/Q10)、行人头/腿型(40km/h)、AEBS、行人AEB+ESF、LKAS、BSD(满足基准最多0.5分)+RCCAS、安全带提醒 SBR。
- **本批未设/未涉及 2 项**:`frontal_small_overlap`(正面小偏置)、`adaptive_highbeam`(自适应远光 AHB)→ 矩阵标「不测」(KNCAP 现行事故预防项为 AEBS/LKAS/BSD/RCCAS/ISA/ESF,无 AHB)。
- **无对应矩阵行的 KNCAP 项**:ISA(智能限速)、ESF(紧急转向,已并入 vru_active 说明)。`compact cars.pdf` 为微型电动车评分实例(参考,非协议来源)。
- 构建脚本:`build_kncap.py`;视图 `ncap_matrix_view.html` / 模块 / 自包含三处 `SYS` 与版本图例已加 K-NCAP。
