...
「层级树」标签页有两种组织方式,可切换:
- **上位法规**(一期原逻辑,业务规则 3.6):基于「上位法规」列构建 框架→技术→细则
  的父子树。数据无该列时此模式为空。
- **法规分类**(二期新增,默认):按业主提供的《法规分类框架》(5 个一级 → 20 个
  二级 → 51 个三级)自动把每条法规归入末级节点。归类依据优先级:
  1. **标准引用匹配**(高置信):法规的国际标准引用(UN R/GTR/FMVSS/GB)命中
     某末级节点的示例标准 → 该节点;
  2. **关键词匹配**:法规名称/编号含末级节点的范围关键词、节点名、或别名表
     (`TAX_ALIAS`,补充排放/召回/防盗/电池/标签等高频词)中的词。
  归不进框架的进「未分类」桶,供补充对标标准或人工核对。分类为**自动建议**,
  以标准引用驱动的最可靠;程序性/框架性法规可能因对标标准是参照项而归类偏差。
